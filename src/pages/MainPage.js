    ...
    const fetchUserStats = async () => {
      try {
        ...
        setUserStats({
          ...
          progress: Math.min(100, Math.round((data.weekSaved / data.goalAmount) * 100)),
          today: data.today,
          weekTotal: data.weekSpent,
          weekSaveAmount: data.weekSaved
        });
      } catch (err) {
        console.error("메인페이지 유저 통계 로딩 실패:", err.response?.data || err.message);
      }
    };
          ...
          <div className="weekly-card">
            ...
            <div className="weekly-stats-box">
              <div className="stats-row">
                ...
                <span className="stats-value">{formatNumber(userStats.weekTotal)}원</span>
              </div>
              <div className="stats-row highlight">
                <span className="stats-label">절약 금액</span>
                <span className="stats-value">{formatNumber(userStats.weekSaveAmount)}원 아꼈어요!</span>
              </div>
            </div>
